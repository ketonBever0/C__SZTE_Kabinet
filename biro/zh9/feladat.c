...
char *foglal(int n)
{
	...
	char **tomb2d = malloc(sizeof(char *) * n);
	for (int i = 0; i < n; i++)
	{
		tomb2d[i] = malloc(sizeof(char) * n);
	}

	// printf("%d\n", n);

	char *tomb1d = malloc(sizeof(char) * (n * n));
...
int zarojel(char *kifejezes)
{
	...
	for (int i = 0; i < hossz; i++)
	{
		...
		// printf("%d %d\n", zarojelKezdve, zarojelZarva);

		// if(kiffejezes[i] != '(' && kiffejezes[i] != ')') {
		// 	if (kifejezes[i] == '*' || kifejezes[i] == '+' || kifejezes[i] == '/' || kifejezes[i] == '-')
		// 	{
		// 		if (kifejezes[i + 1] == '*' || kifejezes[i + 1] == '+' || kifejezes[i + 1] == '/' || kifejezes[i + 1] == '-') {
